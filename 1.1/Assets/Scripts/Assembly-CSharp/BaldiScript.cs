...
public class BaldiScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			antiHearingTime -= Time.unscaledDeltaTime;
		}
		else
		{
			antiHearing = false;
	...
	private void FixedUpdate()
	{
		...
		RaycastHit hitInfo;
		if (Physics.Raycast(base.transform.position + Vector3.up * 2f, direction, out hitInfo, float.PositiveInfinity, 3, QueryTriggerInteraction.Ignore) & (hitInfo.transform.tag == "Player"))
		{
			db = true;
			TargetPlayer();
		}
