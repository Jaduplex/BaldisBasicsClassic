...
public class GameControllerScript : MonoBehaviour
{
	...
	private string[] itemNames = new string[8] { "Nothing", "Energy flavored Zesty Bar", "Yellow Door Lock", "Principal's Keys", "BSODA", "Quarter", "Baldi Anti Hearing Tape", "Alarm Clock" };
	...
	private void Update()
	{
		...
		if (finaleMode && !audioDevice.isPlaying)
		{
			if (exitsReached == 2)
			{
				audioDevice.PlayOneShot(aud_MachineStart, 0.8f);
			}
			else if (exitsReached == 3)
			{
				audioDevice.PlayOneShot(aud_MachineLoop, 0.8f);
			}
		}
	}
	...
	public void ExitReached()
	{
		...
		if (exitsReached == 3)
		{
			audioDevice.PlayOneShot(aud_MachineRev, 0.8f);
		}
	}
