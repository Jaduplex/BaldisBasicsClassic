...
public class BullyScript : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		RaycastHit hitInfo;
		if (Physics.Raycast(base.transform.position + new Vector3(0f, 4f, 0f), direction, out hitInfo, float.PositiveInfinity, 3, QueryTriggerInteraction.Ignore) & (hitInfo.transform.tag == "Player") & bullyRenderer.isVisible & ((base.transform.position - player.position).magnitude <= 20f) & active)
		{
			if (!spoken)
			{
